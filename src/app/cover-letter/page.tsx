...
import { ArrowRight, CheckCircle2, User, UserCheck, Menu } from "lucide-react";
...
export default function CoverLetterTemplatePage() {
  ...
  const handleNext = () => {
    ...
    params.set("withProfileImage", String(withProfileImage));
      ...
      <header className="sticky top-0 z-50 w-full bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
       ...
       <div className="container mx-auto p-4 sm:p-6 lg:p-8">
        <div className="flex flex-col items-center text-center space-y-4 mb-12">
            <h1 className="text-3xl md:text-4xl font-bold tracking-tight font-sans">Design Your Cover Letter</h1>
            <p className="text-base md:text-lg text-muted-foreground max-w-2xl">
                Choose a template and color scheme to create a professional cover letter that gets you noticed.
            </p>
        </div>
        ...
        <div className="mb-12">
            ...
            <div className="flex justify-center items-center gap-4">
                ...
                <Switch
                    ...
                    checked={withProfileImage}
                    onCheckedChange={setWithProfileImage}
                />
                <UserCheck className={cn("h-8 w-8", withProfileImage ? "text-primary" : "text-muted-foreground")} />
            </div>
             <p className="text-center text-muted-foreground mt-2">{withProfileImage ? "Cover letter will include a profile image." : "Cover letter will not include a profile image."}</p>
        </div>
